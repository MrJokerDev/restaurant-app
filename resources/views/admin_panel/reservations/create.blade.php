        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form method="POST" action="{{ route('admin.reservations.store') }}" class="max-w-sm mx-auto">
                        @csrf
                        <div class="mb-5">
                        ...
                        <div class="mb-5">
                            <label for="res_date" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Date</label>
                        ...
                        <div class="mb-5">
                            <label for="table_id" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Select table</label>
                            <select id="table_id" name="table_id" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                                ...
                                @foreach($tables as $table)
                                    <option value="{{ $table->id }}">{{ $table->guest_number }} - {{ $table->name }}</option>
                                @endforeach
                            </select>
                        </div>
